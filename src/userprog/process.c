...
start_process (void *file_name_)
{
  ...
  sema_up(&t->parent->mem_load_lock);
  if (!success){		 
	t->load_flag=true;
	  exit(-1);
	//thread_exit ();
...
process_wait (tid_t child_tid UNUSED) 
{
	...
	if(child_tid <0)
		return -1;
    //elem = list_begin(&t->child);	
	for(elem=list_begin(&t->child); elem!=list_end(&t->child); elem=list_next(elem)){
		tmp = list_entry(elem, struct thread, child_elem);
		if(tmp->tid == child_tid){
            sema_down(&(tmp->child_thread_lock));
	        status = tmp->exit_status;
	        list_remove(&(tmp->child_elem));
	        sema_up(&(tmp->mem_preserve));
            return status;
		}
	}
	...
	if(tmp == NULL || t== NULL)
		return -1;
/*
	sema_down(&(child->child_thread_lock));
	exit_status = child->exit_status;
	list_remove(&(child->child_elem));
	sema_up(&(child->mem_preserve));
...
process_exit (void)
{
  ...
  sema_up(&(cur->child_thread_lock));
  sema_down(&(cur->mem_preserve));
}
...
void stack_arguments(const char* file_name, void** esp)
{
	...
	for(int i=0; i<size_of_args && com != NULL ; i++){
		//if(!com) break;
		args[i] = com;
		com = strtok_r(NULL, " ", &ptr);
	}
  int i;
  //unsigned int len; //length of arguments
  uint32_t argv[size_of_args+1];
  //printf("cur addr %n \n",(uint32_t*)*esp); 
  for( i = size_of_args-1; i>=0; i--){
    //printf("args[%d] : %s\n",i,args[i]); 
